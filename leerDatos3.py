...
FILE_SENO = 'seno.dat' # Archivo para los datos del seno
FILE_VENTANA = 'ventana.dat' # Archivo para los datos de la ventana
...
try: # Intenta esto
    ...
    while True: # Bucle infinito
        ...
        # --- LÓGICA DEL SENO ---
        if "###INICIO_SENO###" in linea: # Detecta la marca de inicio de los datos del seno
            print(f"\n>>> Detectado inicio de datos del SENO. Grabando en {FILE_SENO}...") # Mensaje en la terminal
            file_handle = open(FILE_SENO, 'w') # Crea o abre el archivo de datos del seno
            grabando = True # Cambia el valor de bandera de control para guardar los datos
            continue # Regresa al inicio del while para no guardar la etiqueta
            
        if "###FIN_SENO###" in linea: # Detecta la marca de fin de datos del seno
            print(f">>> Fin datos de datos del SENO. Archivo cerrado.") # Mensaje en la terminal
            if file_handle: file_handle.close() # Guarda y cierra el archivo creado
            grabando = False # Cambia el valor de la bandera de control
            continue # Regresa al inicio del while para no guardar la etiqueta

        # --- LÓGICA DE LA VENTANA ---
        if "###INICIO_VENTANA###" in linea: # Detecta la marca de inicio de los datos del seno
            print(f"\n>>> Detectado inicio de datos de LA VENTANA. Grabando en {FILE_VENTANA}...") # Mensaje en la terminal
            file_handle = open(FILE_VENTANA, 'w') # Crea o abre el archivo de datos de la ventana
            grabando = True # Cambia el valor de bandera de control para guardar los datos
            continue # Regresa al inicio del while para no guardar la etiqueta
            
        if "###FIN_VENTANA###" in linea: # Detecta la marca de fin de datos de la ventana
            print(f">>> Fin datos de datos de la VENTANA. Archivo cerrado.") # Mensaje en la terminal
            if file_handle: file_handle.close() # Guarda y cierra el archivo creado
            grabando = False # Cambia el valor de la bandera de control
